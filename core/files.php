...
class Files {
     ...
     * 
     * @param int $time
     * The new modification time in Unix timestamp.
     * 
     * @return bool
     * True if the modification time was set successfully, false otherwise.
     */
    public static function setModTime(string $path, int $time): bool {
        $fullPath = join(DIRECTORY_SEPARATOR, [$_SERVER['DOCUMENT_ROOT'], $path]);
        return touch($fullPath, $time);
    }

    /**
     * Returns a version number as a query parameter that is added to
     * a file for cache busting. The version number is the modification
     * time of the file.
     *
     * @param string $path     
     * The file path.
     * 
     * @return string|null               
     * The extended string, or null if the file doesn't exist.
     */
    public static function getVersion(string $path): ?string {
        $modified = Files::getModTime($path);
        return $modified !== null ? sprintf("%s?version=%d", $path, $modified) : null;
    }
    ...
    private static function includeFilter(string $condition = null): bool {
        ...
        // Return true if the current URL matches the include condition; otherwise, 
        // return false (include on specific pages)
        return Pages::this() === $condition;
    }
     ...
     */
    private static function reference(string $file, string $type, ?string $conditions = null): string {
        
        // Get file modification time as the version number:
        $version = Files::getVersion($file);
